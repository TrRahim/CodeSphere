...
const LanguageSelector = ({ hasAccess }: { hasAccess: boolean }) => {
  ...
  const setLanguage = useCodeEditorStore((s) => s.setLanguage);
  ...
  return (
    <div className="relative" ref={dropDownRef}>
      <motion.button
        ...
        className={`group relative flex items-center gap-3 px-4 py-2.5 bg-[#1e1e2e]/80 rounded-lg transition-all duration-200 border border-gray-800/50 hover:border-gray-700 ${!hasAccess && language !== "cpp" ? "opacity-50 cursor-not-allowed" : ""}`}
      >
        <div
